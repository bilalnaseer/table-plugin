	...
	else
{
	...
	*/
	...
	foreach ($get_id as $id) {
		$shortcode = '[cs_table id="' . $id->id . '"]';
	}
	$wpdb->update( $table_name, array( 'shortcode' => $shortcode ), array( 'tablename' => $name ) );	

	header('Location: ' . get_bloginfo('wpurl') . '/wp-admin/tools.php?page=cool_tables&updated=true');
	exit;
	}
}
...
if ( empty($_GET) || !wp_verify_nonce($nonce, 'duplicate-table' ) )
{
   ...
   exit;	
	...
	$copy = get_option('cool_tables_copy');
	global $wpdb;
	$table_name = $wpdb->prefix . "cool_tables";
	...
	foreach ($result as $results) {
		$id = $results->id;
		$shortcode = '[cs_table id="' . $id . '"]';
	}
	$copy = $copy+1;
	update_option("cool_tables_copy", $copy);
	...
	else
{
	...
	header('Location: ' . get_bloginfo('wpurl') . '/wp-admin/tools.php?page=cool_tables&renamed=' . $id);
	exit;	
	...
	else
{
	...
	if ($handle = @fopen($csvurl, "r") !== FALSE)
	{
		...
		*/
		...
		header('Location: ?page=cool_tables');
		exit;
	}
	elseif ($handle = @fopen($csvurl, "r") === FALSE) //could not find file
	{
		$errormessage = '0';		
		header('Location: ?page=cool_tables&action=ws_import_table&error=' . $errormessage . '');
		exit;
	}
...
{	 
	...
	$wpdb->update( $table_name, array( 'headlines' => $headlines, 'content' => $content ), array( 'id' => $id ) );	
	header('Location: ' . get_bloginfo('wpurl') . '/wp-admin/tools.php?page=cool_tables&action=edit_table&id=' . $id . '');
	exit;	
}
}
...
if ( empty($_GET) || !wp_verify_nonce($nonce, 'organize_table' ) )
{
   ...
   exit;	
	...
	header('Location: ?page=cool_tables&action=edit_table&id=' . $id . $column_message);
	exit;	
}
}
/*
	...
	else
{
	...
	header('Location: ' . get_bloginfo('wpurl') . '/wp-admin/tools.php?page=cool_tables&action=edit_style&id=' . $id . '');
	exit;	
}
}
	...
	else
{
	...
	header('Location: ' . get_bloginfo('wpurl') . '/wp-admin/tools.php?page=cool_tables&action=edit_style&id=' . $id . '&settings=advanced');
	exit;	
}
}	
?>