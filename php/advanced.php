<?php
	...
	<h2 class="nav-tab-wrapper">
		...
		<a class="nav-tab" href="' . get_bloginfo('wpurl') . '/wp-admin/tools.php?page=cool_tables&action=edit_table&id=' . $this_id . '">Edit Table Content</a>
		<a class="nav-tab nav-tab-active" href="' . get_bloginfo('wpurl') . '/wp-admin/tools.php?page=cool_tables&action=edit_style&id=' . $this_id . '">Edit Table Structure & Style</a>
	</h2>
	
	<h2>Style for ' . $name . '</h2>
	
	<h3 class="nav-tab-wrapper">
		<a class="nav-tab" href="' . get_bloginfo('wpurl') . '/wp-admin/tools.php?page=cool_tables&action=edit_style&id=' . $this_id . '&settings=basic">Basic Settings</a>
		<a class="nav-tab nav-tab-active" href="' . get_bloginfo('wpurl') . '/wp-admin/tools.php?page=cool_tables&action=edit_style&id=' . $this_id . '&settings=advanced">Custom skin colors</a>
	</h3>
					...
					<div id="namediv" class="stuffbox">
						...
						if ($style != 'custom') {
							...
							<p class="admin-activate-advanced">
								Important: You must activate custom skin first in <a href="' . get_bloginfo('wpurl') . '/wp-admin/tools.php?page=cool_tables&action=edit_style&id=' . $this_id . '&settings=basic">basic settings</a>
							</p>'; 
						} else {
						echo '
						<div id="admin-left">
							...
							<p>
								<script type="text/javascript" src="' . get_option('siteurl') . '/wp-content/plugins/cool-tables/js/jscolor/jscolor.js"></script>
